...
export const CoachPanel: React.FC<CoachPanelProps> = ({ className = '' }) => {
  ...
  return (
    <div className={`p-4 rounded-2xl bg-slate-900/60 border border-slate-800 space-y-3.5 ${className}`}>
      {/* Header: Coach + Badge */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-xl bg-gradient-to-tr from-emerald-600/30 to-teal-500/30 border border-emerald-500/40 flex items-center justify-center text-emerald-400">
            <Bot className="w-4 h-4" />
          </div>
          <div>
            ...
            <p className="text-[10px] text-slate-400">Move {currentReport.ply} of {reports.length}</p>
          </div>
        </div>
        ...
        {meta && (
          <div
            className="flex items-center gap-1.5 px-2.5 py-1 rounded-xl text-slate-950 font-black text-xs shadow-md"
      ...
      <div className="p-3.5 rounded-xl bg-slate-950/70 border border-slate-800 text-xs text-slate-200 leading-relaxed shadow-inner">
      ...
      <div className="grid grid-cols-2 gap-2 pt-1">
        <div className="p-2 rounded-lg bg-slate-950/50 border border-slate-800 text-center">
          <span className="text-[10px] text-slate-500 uppercase font-semibold">Move Accuracy</span>
          <p className="text-xs font-bold font-mono text-slate-200">
            {currentReport.accuracy.toFixed(1)}%
          </p>
        </div>
        <div className="p-2 rounded-lg bg-slate-950/50 border border-slate-800 text-center">
          <span className="text-[10px] text-slate-500 uppercase font-semibold">Advantage Loss</span>
          <p className="text-xs font-bold font-mono text-slate-200">
